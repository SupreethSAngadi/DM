...
# Read CSV data file
mydata = read.csv("C:\\Users\\ShivarajRs\\Desktop\\PRoject\\1dm.csv")

# See first 10 observations
head(mydata, n=10)

# Split data
dt <- split(mydata$Month,mydata$commudity)
...
#itemFrequencyPlot(dt2,topN = 1)


#transactions <- transactions[,-pmatch("Jan", colnames(transactions))]
#itemsets <- apriori(transactions, parameter = list(target = "frequent",supp=0.001, minlen = 2, maxlen=4))
...
#aggregated data
rules = apriori(dt2, parameter=list(support=0.5, confidence=0.8))
rules = apriori(dt2, parameter=list(support=0.5, confidence=0.8, minlen = 3))
rules = apriori(dt2, parameter=list(support=0.5, confidence=0.8, maxlen = 4))
